...
def rle_compress(arr: th.Tensor, buffer: RawBuffer, elem_type: th.dtype = th.uint8,
                 runs_type: th.dtype = th.uint16) -> tuple[int, int, int]:
...
def rle_decompress(buffer: RawBuffer, pos_runs: int, pos_elem: int, run_length: int, elem_type: th.dtype,
                   runs_type: th.dtype, arr_configs: dict) -> th.Tensor:
