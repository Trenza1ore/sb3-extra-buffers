...
Browse the examples in the
`repository examples directory <https://github.com/Trenza1ore/sb3-extra-buffers/tree/main/examples>`__.

The typical on-policy pattern is:
...
The typical off-policy pattern is:
...
These results are copied from the README and are included here so that the docs
capture more than the API surface.

PPO on ``PongNoFrameskip-v4`` with ``rle-jit`` and no frame stack:
...
PPO on ``MsPacmanNoFrameskip-v4`` with ``rle-jit`` and frame stack ``4``:
...
DQN on ``MsPacmanNoFrameskip-v4`` with ``rle-jit`` and frame stack ``4``:
...
If you use this project in research or published work, cite:
